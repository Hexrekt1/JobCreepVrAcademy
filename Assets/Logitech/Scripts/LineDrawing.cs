...
public class LineDrawing : MonoBehaviour
{
    ...
    private List<float> _currentLineWidths = new(); //list to store line widths
    ...
    private void AddPointToLine(Vector3 position, float pressure)
    {
        ...
        _currentLineWidths.Add(Math.Max(pressure * _maxLineWidth, _minLineWidth));
   ...
   private GameObject FindClosestLine(Vector3 position)
{
    ...
    foreach (var line in _lines)
    {
        ...
        {
            var point = FindNearestPointOnLineSegment(lineRenderer.GetPosition(i), lineRenderer.GetPosition(i + 1), position);
            var distance = Vector3.Distance(point, position);
            
            if (distance < closestDistance && distance < highlightThreshold)
            {
    ...
    // Ensure a value is returned, even if no line is found
    return closestLine;
}
    ...
    private void MoveHighlightedLine()
    {
        ...
        {
            newPositions[i] = rotation * (_originalLinePositions[i] - _grabStartPosition) + stylusHandler.Stylus.inkingPose.position;
        }
    ...
    private void HighlightLine (GameObject line)
    {
        ...
        var lineRenderer = line.GetComponent<LineRenderer>();
        _cachedColour = lineRenderer.material.color;
    ...
    {
        var lineRenderer = line.GetComponent<LineRenderer>();
        lineRenderer.material.color = _cachedColour;
...
{
    if (!_highlightedLine) return; // Ensure there's a line to grab

    _grabStartPosition = stylusHandler.Stylus.inkingPose.position; // Store starting position
    _grabStartRotation = stylusHandler.Stylus.inkingPose.rotation; // Store starting rotation

    var lineRenderer = _highlightedLine.GetComponent<LineRenderer>();
    _originalLinePositions = new Vector3[lineRenderer.positionCount];
    lineRenderer.GetPositions(_originalLinePositions); // Save the current line positions

    _currentMode = AppMode.Grabbing; // Switch mode to Grabbing
}
private void UpdateLineColor()
{
    if (tipIndicator != null)
    {
        // Update the tip color to match the selected line color
        tipIndicator.material.color = lineColor;
    }
}
private Vector3 FindNearestPointOnLineSegment(Vector3 start, Vector3 end, Vector3 point)
{
    // Calculate the line vector and the vector from start to the point
    Vector3 lineVector = end - start;
    Vector3 pointVector = point - start;

    // Project pointVector onto lineVector to find the closest point
...
private void StopGrabbingLine()
{
    ...
    // Reset the mode back to Idle
    _currentMode = AppMode.Idle;

    // Clear any references related to grabbing
    _grabStartPosition = Vector3.zero;
    _grabStartRotation = Quaternion.identity;
    _originalLinePositions = null;

    Debug.Log("Stopped grabbing the highlighted line.");
}
